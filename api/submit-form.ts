...
function renderList(guests: Guest[]): string {
  return guests.map(entryTemplate).join("\n\n---\n\n");
}
...
export default async (req: NowRequest, res: NowResponse) => {
  ...
  try {
    ...
    res.json({ redirect: `https://github.com/${process.env.REPO_OWNER}` });
  } catch (err) {
    console.error(err);
    res.json({
      error: "Something weird happened and your entry wasn't added!",
    });
  }
};
